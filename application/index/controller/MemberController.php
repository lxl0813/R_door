...
use app\index\model\FormModel;
use app\index\model\FormTypeModel;
...
class MemberController extends CommonController{
    ...
    public function more(){
        ...
        $form = FormModel::FormAll($form_id);                   //查询该公司下的该报表类型下的所有报表
        $form_type = FormTypeModel::where(['id'=>$form_id['form_type_id']])->value('form_type'); //查询报表类型
        $company   = CompanyModel::CompanyWhereFind("company",['id'=>$form_id['form_company_id']]);//查询该公司
    ...
    public function form_index(){
        //查询所有公司
        $company = CompanyModel::CompanyAll();
        //查询所有类型
        ...
        if($datas){
            ...
            if(empty($form)){
                ...
                foreach($form as $k=>$v){
                    $form[$k]['type'] = \app\admin\model\FormTypeModel::FormTypeWhereFind("form_type",['id'=>$v['form_type_id']]);
                    $form[$k]['company'] = \app\admin\model\CompanyModel::CompanyWhereGet(["id"=>$v['form_company_id']],"company");
                    $form[$k]['time'] = date("Y-m-d H:i:s",$v['time']);
                }
            }
        }else{
            //查询所有流水账单（显示当月的流水账单）
            $form = \app\admin\model\FormModel::FormSelect();
            foreach($form as $k=>$v){
                $form[$k]['type'] = \app\admin\model\FormTypeModel::FormTypeWhereFind("form_type",['id'=>$v['form_type_id']]);
                $form[$k]['company'] = \app\admin\model\CompanyModel::CompanyWhereGet(["id"=>$v['form_company_id']],"company");
                $form[$k]['time'] = date("Y-m-d H:i:s",$v['time']);
    ...
    public function water_index(){
        $company= \app\admin\model\CompanyModel::CompanyAll();
        $no="1";
        //条件查询
        $data=input('param.');
        if($data){
