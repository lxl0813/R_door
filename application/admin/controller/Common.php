...
class Common extends Controller{
    ...
    public function company(){
        return Company::CompanyAll();
    }

    //报表类型查询
    public function type(){
        return FormType::FormTypeSelect();
    }
    ...
    public function getMenu(){
        ...
        if(in_array($admin_name,config("app.super_admin"))){
            //取所有的权限
            $menu=(new Node())->where("pid",0)->all()->toArray();
            foreach($menu as $k=>$v){
            ...
            //查询管理员的所拥有角色
            $role_id    =   (new AdminRoles())->where('admin_id',$admin_id)->column('role_id');
            $node_id    =   (new RoleNode())->where('role_id','in',$role_id)->column('node_id');
            $menu       =   (new Node())->where('id','in',$node_id)->where('pid','0')->field('controller,action,node')->select()->toArray();
            foreach($menu as $k=>$v){
                $menu[$k]['url']=$v['controller']."/".$v['action'];
            }
        }
