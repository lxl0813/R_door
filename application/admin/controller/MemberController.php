...
class MemberController extends CommonController{
    public function index(){
        if(request()->isGet()){
            //查询会员信息
            $member = \app\admin\model\MemberModel::MemberSelect();
    ...
    public function add(){
        ...
        if(request()->isPost()){
            ...
            $data['time'] = time();
            $member = \app\admin\model\MemberModel::MemberAdd($data);
            if($member){
    ...
    public function delete(){
        ...
        $member_id = \app\admin\model\MemberModel::MemberDelete($member);
        if($member_id){
