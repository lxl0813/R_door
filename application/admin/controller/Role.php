...
class Role extends Common{
    ...
    {
        //查询所有角色（父子结构的数组）
        $roles  =   (new \app\admin\model\Role())::select();
    ...
    public function add()
    {
        if(request()->isGet()){
            //权限查询（父子结构）
            $node   =   new Node();
            $nodes  =   (new DataToTree())->getToTree($node::where('status','1')->select()->toArray());
            //var_dump($nodes);exit;
        ...
        if(request()->isPost()){
            ...
            try {
                ...
                \app\admin\model\Role::addNodeRole($role_id,$node_id);
                // 提交事务
                Db::commit();
            } catch (\Exception $e) {
                // 回滚事务
